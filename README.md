# CyberKit

A modular, zero-dependency beginner cybersecurity sandbox implementation optimized for client-side cross-platform utility. CyberKit offers simultaneous distribution pipelines via native Terminal CLI interface and lightweight single-page application structure[cite: 1, 2].

## Feature Components

* **Password Strength Assessment Matrix**: Algorithmic scoring evaluating structural entropy constraints, tracking key space configurations (uppercase, lowercase, integers, symbol pools) intersected with known standard dictionary exclusions[cite: 1, 2].
* **One-Way Cryptographic Digests**: Immediate translation engine generating hash variants (MD5, SHA-1, SHA-256, SHA-512) leveraging native computing modules and browser abstraction Web Crypto APIs[cite: 1, 2].
* **Caesar Shift Transformation Module**: Letter-substitution cipher executing custom space shifts along with key space depletion configurations [Brute Force auto-cracking](cite: 1, 2).

## Operational Framework Execution

### Command Line Interface

Execute the Python environment module using your local shell instance:

```bash
python app.py
